    ...
    <Box height='100%' >
      <Box display='flex' flexDirection='column' justifyContent='center' alignItems='center' px={{ base: '30px', md: '70px' }}>
          ...
          <Box width='60%' display='flex' justifyContent='flex-end' flexDirection='column' alignItems='start' >
            ...
            <Box my='20px' w='40%'>
              <InputGroup size='md'>
                ...
                />
                <InputRightElement px='3px' display='flex' my='5.5px' alignItems='center' justifyContent='center' width='8.5rem'>
        ...
        <Box width='72%' display='flex' flexDirection='column' justifyContent='center' alignItems='center'>
          <Text width='100%' textAlign='left' my='10px' fontSize='35px' fontWeight='900' as='h1'>Why learners are using Project Partner Finder</Text>
          <Box textAlign='left' width='100%' my='15px' as='p'>
            <Text width='52rem'>
        ...
        <Box my='10px' flexWrap='wrap' display='flex' alignItems='center' gap='0'>
          {data.map((fea, key) => (
            <Box cursor='pointer' transition='all' transitionDuration='0.3s' h='260px' p='8px' py='12px' _hover={{ boxShadow: '0px 0px 10px 0px #e5e5e5' }} rounded='md' w='200px'>
              <img width='190px' src={fea.img} alt="" />
              <Box my='15px' as='p'>
                <Text fontWeight='600' as='h3'>{fea.title}</Text>
                <Text fontSize='13px' as='p'>{fea.para}</Text>
              </Box>
            </Box>
          ))}
        </Box>
        <Box my='15px' width='72%'>
          <Text width='100%' textAlign='left' fontSize='30px' fontWeight='700'>How it works</Text>
          <Box my='10px' display='flex' flexWrap='wrap' justifyContent='space-between' alignItems='center' gap='10'>
            {data2.map((wor, key) => (
              <Box cursor='pointer' h='200px' border={'solid 1px #b5b7ba'} p='12px' boxShadow={'0px 0px 10px 0px #e5e5e5'} rounded='10px' w='200px'>
                <Box as='span'>{wor.img}</Box>
                <Box my='15px' as='p'>
                  <Text fontWeight='700' as='h3'>{wor.title}</Text>
                  <Text fontSize='13px' as='p'>{wor.para}</Text>
                </Box>
              </Box>
            ))}
          </Box>
        ...
        <Box my='10px' display='flex' justifyContent='center' flexDirection='column' alignItems='center'>
          <Text my='5px' fontSize='35px' fontWeight='900' as='h1'>
